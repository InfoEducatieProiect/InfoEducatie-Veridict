...
import { getSubmissionsForAssignment } from "@/lib/supabase/queries"

/**
 * Batch analysis pipeline: submissions → cosine + phrase Jaccard →
 * analysis_runs / analysis_scores / peer_matches (server-side Supabase session).
 */
export async function POST(request: Request) {
  try {
    ...
    const submissionsRaw = await getSubmissionsForAssignment(assignmentId, supabase)
